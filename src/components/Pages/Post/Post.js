...
const Post = ({ navigation, route, imgUri = "file:///storage/emulated/0/Download/PHOTO_20200618_175433-01.jpg" }) => {
  ...
  try {
    ...
    console.log("CreatePost fail get route.params.imageData", error);
  }
  ...
  const onSave = () => {
    console.log("onpressss", navigation, route);
    ...
    <Layout style={styles.container}>
      ...
      <Layout style={styles.container}>
        <Layout level="2" style={styles.panelImg}>
          {imgUri !== "" && <Image style={styles.theImg} source={{ uri: imgUri }} resizeMode="contain" ></Image>}
        </Layout>
        ...
        <Layout style={styles.panelButton}>
          <CButton onPress={onSave} icon="star" textButton="Save" />
        </Layout>
      </Layout>
    </Layout>
  );
